...
def extract_info(url, ext, playlist):
    ...
    Args:
        url (str): YouTube URL.
        ext (str): File extension.
        playlist (str or None): Playlist name.

    Returns:
        str or False or None: Extraction status.
    """
    ...
    if isinstance(playlist_dict, dict) and "entries" in playlist_dict:
        if not playlist:
...
def init(ext, start, end):
    ...
    Args:
        ext (str): File extension.
        start (int): Download playlist starting from this index.
        end (int or None): Download playlist ending at this index.
    """
    tmp["start"] = start
...
def download_video():
    ...
    if tmp["playlist"]:
        ...
        tmp["ydl_opts"][
            "outtmpl"
        ] = f"{CONFIG['outdir']}/{tmp['playlist']}/%(title)s-%(id)s.%(ext)s"
    else:
        path = f"{CONFIG['outdir']}/*"
        tmp["ydl_opts"]["outtmpl"] = f"{CONFIG['outdir']}/%(title)s-%(id)s.%(ext)s"
...
def write_diff():
    ...
    if tmp["add"] or tmp["del"] or tmp["pop"]:
        ...
        with open(path, "a", encoding="utf8") as diff:
            ...
            diff.write(
                f"@ {curr_time} (+{len(tmp['add'])} -{len(tmp['del'])} !{len(tmp['pop'])})\n"
            )
            for file in tmp["add"]:
